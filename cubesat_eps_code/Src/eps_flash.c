/* ============================================================
 *  CubeSat EPS — Flash persistence
 *  System state saving and restoration.
 *
 *  Target MCU: STM32G474RETx
 *    Flash: 512 KB (0x08000000 – 0x0807FFFF)
 *    Page size: 2 KB, number of pages: 256
 *    Two banks of 128 pages each (Bank 1: 0-127, Bank 2: 128-255)
 *    We use the last page of Bank 2 (page 255)
 *    Address: 0x08000000 + 255 * 2048 = 0x0807F800
 * ============================================================ */
...
uint8_t EPS_Flash_DiagCRC(void)
{
    SystemLog log;
    memcpy(&log, (const void*)FLASH_LOG_ADDRESS, sizeof(log));
    uint32_t calc = Log_CRC(&log);
    return (calc == log.crc) ? 1 : 0;
}
