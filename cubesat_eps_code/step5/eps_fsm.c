...
static EPS_State Run_PowerCheck(void)
{
    ...
    if (s_charging && !s_full_charge)
    return STATE_CHARGING;
    if (s_vbat >= VBAT_NORMAL)
    return STATE_NORMAL;
    if (s_vbat >= VBAT_LOWPOWER)
...
void FSM_Update(float vbat, float vsol)
{
    ...
    switch (s_current)
    {
        case STATE_BOOT: next = Run_Boot(); break;
        case STATE_POWER_CHECK: next = Run_PowerCheck(); break;
        case STATE_CHARGING: next = Run_Charging(); break;
        case STATE_NORMAL: next = Run_Normal(); break;
        case STATE_LOW_POWER: next = Run_LowPower(); break;
        case STATE_CRITICAL: next = Run_Critical(); break;
        case STATE_HIBERNATE: next = Run_Hibernate(); break;
        case STATE_ERROR: next = Run_Error(); break;
    ...
    if (next != s_current)
    {
        ...
        switch (s_current)
        {
            ...
            case STATE_POWER_CHECK: Enter_PowerCheck(); break;
            case STATE_CHARGING: Enter_Charging(); break;
            case STATE_NORMAL: Enter_Normal(); break;
            case STATE_LOW_POWER: Enter_LowPower(); break;
            case STATE_CRITICAL: Enter_Critical(); break;
            case STATE_HIBERNATE: Enter_Hibernate(); break;
            case STATE_ERROR: Enter_Error(); break;
            default: break;
        }
    }
}
